...
public class StatsReportPanel extends JPanel {
	...
	private String[] education; // Education array (No Degree, High School or
								// Equivalent, Some College, Bachelors, Masters,
								// Doctorate)
	private String[] race; // Race array (Asian, Latino, Black/African-American,
							// Pacific Islander, Caucasian, Other
	private String[] patientType; // Patient (Pediatric, Vascular, Neurologic,
									// Gynocology, Orthopedic, Other)
	...
	private class SubmitListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			for (int i = 0; i < count; i++) {
				if (education[i].equalsIgnoreCase("No Degree"))
					numOfND++;
						...
						.equalsIgnoreCase("High School or Equivalent"))
					numOfHS++;
			...
			// generate the report
			String statsReport = "Total number of patients in time span: "
					+ count + "\n"
					+ "-----------------------------------------\n"
					+ "Gender statistics: \n" + "Number of males: "
					+ numOfMales + " (" + numOfMales / (double) count * 100
					+ "%)\n" + "Number of females: " + numOfFemales + " ("
					+ numOfFemales / (double) count * 100 + "%)\n"
					+ "-----------------------------------------\n"
					+ "Marital status statistics: \n" + "Number of singles: "
					+ numOfSing + " (" + numOfSing / (double) count * 100
					+ "%)\n" + "Number of married: " + numOfMar + " ("
					+ numOfMar / (double) count * 100 + "%)\n"
					+ "-----------------------------------------\n"
					+ "Education statistics: \n" + "Number of no degrees: "
					+ numOfND + " (" + numOfND / (double) count * 100 + "%)\n"
					+ "Number of high school diplomas: " + numOfHS + " ("
					+ numOfHS / (double) count * 100 + "%)\n"
					+ "Number of some college experience: " + numOfSC + " ("
					+ numOfSC / (double) count * 100 + "%)\n"
					+ "Number of Bachelors: " + numOfBach + " ("
					+ numOfBach / (double) count * 100 + "%)\n"
					+ "Number of Masters: " + numOfMast + " ("
					+ numOfMast / (double) count * 100 + "%)\n"
					+ "Number of Doctorate: " + numOfDoc + " ("
					+ numOfDoc / (double) count * 100 + "%)\n"
					+ "-----------------------------------------\n"
					+ "Race statistics: \n" + "Number of Caucasians: "
					+ numOfCauc + " (" + numOfCauc / (double) count * 100
					+ "%)\n" + "Number of Latinos: " + numOfLat + " ("
					+ numOfLat / (double) count * 100 + "%)\n"
					+ "Number of Asians: " + numOfAsian + " (" + numOfAsian
					/ (double) count * 100 + "%)\n"
					+ "Number of Black/African-Americans: " + numOfBAA + " ("
					+ numOfBAA / (double) count * 100 + "%)\n"
					+ "Number of Pacific Islanders: " + numOfPacIs + " ("
					+ numOfPacIs / (double) count * 100 + "%)\n"
					+ "Number of Others: " + numOfOtherRace + " ("
					+ numOfOtherRace / (double) count * 100 + "%)\n"
					+ "-----------------------------------------\n"
					+ "Patient type statistics: \n"
					+ "Number of vascular patients: " + numOfVasc + " ("
					+ numOfVasc / (double) count2 * 100 + "%)\n"
					+ "Number of pediatric patients: " + numOfPed + " ("
					+ numOfPed / (double) count2 * 100 + "%)\n"
					+ "Number of neurologic patients: " + numOfNeuro + " ("
					+ numOfNeuro / (double) count2 * 100 + "%)\n"
					+ "Number of gynocology patients: " + numOfGyno + " ("
					+ numOfGyno / (double) count2 * 100 + "%)\n"
					+ "Number of orthopedic patients: " + numOfOrtho + " ("
					+ numOfOrtho / (double) count2 * 100 + "%)\n"
					+ "Number of other patients: " + numOfOtherType + " ("
					+ numOfOtherType / (double) count2 * 100 + "%)\n"
					+ "-----------------------------------------";
