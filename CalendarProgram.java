...
public class CalendarProgram {
	...
	public static void main(String args[]) {
		...
		try {
			...
			while (rs.next()) {
				Appointment a1 = new Appointment(rs.getString("docName"),
						rs.getString("month"), rs.getString("year"),
						rs.getString("day"), rs.getString("time"),
						rs.getString("docType"), rs.getInt("row"),
						rs.getInt("col"), rs.getInt("patientID"),
						rs.getInt("appointmentsID"));
				person.add(a1);
				id = rs.getInt(1);
			}
		...
		fmain.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // Close when X is
																// clicked
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (value != null) {
				if (Integer.parseInt(value.toString()) == rday
						&& currentMonth == rmonth && currentYear == ryear) { // Today
	...
	static class mouseCont extends MouseAdapter {
	// use this to access get
	// another frame pulled up
	// to note down appointment
	// time and what not
		public void mouseClicked(MouseEvent e) {
			int row = Cal.getSelectedRow();// this will give you the row number
			int col = Cal.getSelectedColumn();// this will give you the column
												// number
			...
			if (row != -1
					&& col != -1
					&& Cal.getValueAt(row, col) != null
					&& existsA(Integer.toString(mon), currentYear, Cal.getValueAt(row, col).toString()) == -1) {// this now will only activate when there is an actual date associated with that day
	...
	static String[] input(final int d, final int m, int y, int d1) {// input
																	// frame for
																	// schedule
		...
		submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					...
					stat.executeUpdate("INSERT INTO appointments (appointmentsID, patientID, docType, docName, day, month, year, time, row, col)"
							+ " VALUES ( "
							+ id
							+ ", "
							+ patientID
							...
							+ currentCol 
							+ ")");
					Appointment list = new Appointment(name, d[0], d[2], d[1], time, docT, currentRow, currentCol, patientID, id);
					person.add(list);
				} 
				catch (SQLException e)
				{
