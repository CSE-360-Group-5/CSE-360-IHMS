  ...
  private JButton filledButton;
	...
	public ViewPrescriptions() {
		...
		try {
			...
			rs = statement.executeQuery("SELECT * FROM prescriptions");
            ...
            while (rs.next()) { // loads the prescriptions from the database and adds them to the list
                ...
                prescript.setPrescriptName(rs.getString("prescriptName"));
                prescript.setPrescriptID(rs.getString("prescriptID"));
                prescript.setType(rs.getString("type"));
                prescript.setDate(rs.getString("date"));
                prescript.setFilled(rs.getString("filled"));

                patientID = prescript.getPatientID(); // sets patient name
		...
		prescriptionsArea = new JTextArea(17,35);
	...
	private class ListListener implements ListSelectionListener {
		...
		public void valueChanged (ListSelectionEvent event) {
			...
			nameField.setText(prescript1.getPatientID());
			dateField.setText(prescript1.getDate());
			prescriptionsArea.setText(prescript1.toString());
		}
	}
  ...
  // Button listener
	filledButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				Prescription prescript2 = new Prescription();
				prescript2 = (Prescription) prescriptionsList.getSelectedValue();

				try {
					statement = conn.createStatement();
					statement.executeUpdate("DELETE FROM prescriptions WHERE prescriptID='" + prescript2.getPrescriptID() + "';");
